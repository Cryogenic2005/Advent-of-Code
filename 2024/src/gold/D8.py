    ...
    for nodes in node_map:
        for i in range(len(node_map[nodes])):
            for j in range(i + 1, len(node_map[nodes])):
                ...
                next_antinode = (node1[0], node1[1])
                while isValid(next_antinode[0], next_antinode[1], lines):
                    antinode[next_antinode[1]][next_antinode[0]] = True
                    next_antinode = (next_antinode[0] + dx, next_antinode[1] + dy)
                
                next_antinode = (node2[0], node2[1])
                while isValid(next_antinode[0], next_antinode[1], lines):
                    antinode[next_antinode[1]][next_antinode[0]] = True
                    next_antinode = (next_antinode[0] - dx, next_antinode[1] - dy)
